...
- `read` - Read control theory papers and Kuramoto model implementations
- `write` - Save control configurations and synchronization analysis results
- `exec` - Run Python/MATLAB scripts for Kuramoto network simulation and control

## Instructions for Agents

Follow these steps when helping users with Kuramoto network synchronization:

1. **Identify the oscillator network**: Determine the network topology and oscillator parameters
2. **Apply complex-valued embedding**: Transform phase dynamics to linear state space
3. **Design control law**: Select the appropriate control strategy (switched, sliding-mode, or MIMO)
4. **Verify synchronization**: Check phase locking and finite-time convergence

## Examples

### Example 1: Switched Feedforward Control

```
User: "设计耦合振子网络的同步控制器"

Execute:
1. Model the Kuramoto network with complex-valued representation
2. Design the switched feedforward control law
3. Simulate the synchronization dynamics
4. Verify finite-time phase locking
```

### Example 2: Sliding-Mode Synchronization

```
User: "用滑模控制实现Kuramoto网络同步"

Execute:
1. Formulate the MIMO sliding-mode control problem
2. Design the sliding surface and control law
3. Implement the controller in simulation
4. Analyze robustness to disturbances
```