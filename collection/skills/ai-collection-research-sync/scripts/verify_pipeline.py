...
def check_ai_collection_sync(skill_name: str) -> dict:
    ...
    ai_collection_path = (
        Path("/Users/hiyenwong/ai_github/ai_collection/collection/skills") / skill_name
    )
...
def check_index_md(skill_names: list) -> dict:
    ...
    result = {"index_exists": index_path.exists(), "skills_found": []}
...
def check_obsidian_notes(arxiv_ids: list) -> dict:
    ...
    result = {"obsidian_path_exists": obsidian_path.exists(), "notes_found": []}
            ...
            for arxiv_id in arxiv_ids:
                if arxiv_id in content:
                    result["notes_found"].append(
                        {"arxiv_id": arxiv_id, "file": md_file.name}
                    )
...
def check_knowledge_graph(arxiv_ids: list) -> dict:
    ...
    result = {"kg_exists": kg_path.exists(), "papers_found": []}
    ...
    if result["kg_exists"]:
        try:
            ...
            cursor.execute(
                "SELECT name FROM sqlite_master WHERE type='table' AND name='arxiv_papers'"
            )
            ...
            if result["table_exists"]:
                for arxiv_id in arxiv_ids:
                    cursor.execute(
                        "SELECT id, title, skill_name FROM arxiv_papers WHERE id = ?",
                        (arxiv_id,),
                    )
                    row = cursor.fetchone()
                    if row:
...
def main():
    parser = argparse.ArgumentParser(
        description="Verify ai-collection-research-sync pipeline"
    )
    parser.add_argument(
        "--skills", nargs="+", required=True, help="Skill names to verify"
    )
    parser.add_argument(
        "--arxiv-ids", nargs="+", required=True, help="arXiv IDs to verify"
    )
    ...
    status = (
        "✓" if kg_result["kg_exists"] and len(kg_result["papers_found"]) > 0 else "✗"
    )
