...
| vector_data | BLOB (declared) / TEXT (actual) | **Schema mismatch (verified 2026-06-06)**: Column declared as BLOB but `typeof(vector_data)` returns 'text' containing JSON arrays `[f1, f2, ..., f384]`. **Code must handle both**: `if isinstance(blob, str): vec = json.loads(blob)` else `struct.unpack('384f', blob)`. Always check type before unpacking. Also verify via `PRAGMA table_info(kg_vectors)` before writing vector logic.
