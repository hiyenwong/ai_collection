...
- optimal transport in control systems
- Wasserstein distance applications
- stochastic MPC for swarms
- decentralized area coverage
- density matching problems
...
When working with multi-agent coverage or density-driven optimal control:

1. **Identify the problem type**:
   - Coverage tasks → D²OC algorithm
   - Optimal transport → Wasserstein distance minimization
   - Density matching → Reference distribution alignment

2. **Apply three-stage framework**:
   - Stage 1: Local target selection & optimal control
   - Stage 2: Weight update after movement
   - Stage 3: Weight sharing within communication range

3. **Compute optimal control**:
   - Build Θ and Φ matrices
   - Compute barycenter of local targets
   - Apply closed-form control law

4. **Validate convergence**:
   - Check controllability assumptions
   - Verify noise bounds
   - Confirm Wasserstein distance bound

## Examples

### Example 1: Multi-Agent Coverage
```
User: "用 D2OC 实现 50 个无人机的区域 coverage"

Agent:
1. 识别问题: 多智能体 coverage，随机环境
2. 应用 D²OC 三阶段框架
3. 计算每个代理的最优控制输入
4. 输出控制序列和收敛保证
```
...
```
User: "如何用最优传输理论实现密度匹配？"

Agent:
1. 识别问题: Wasserstein 距离最小化
2. 定义经验分布和参考分布
3. 应用 D²OC 算法
4. 输出匹配结果和误差界
```
