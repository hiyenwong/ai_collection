...
### Feasibility-Driven Penalty Scheduling (arxiv:2606.25117)

For problems with multiple heterogeneous constraints, promote penalty weights from external hyperparameters to internal variational parameters:

- **Lambda-lr-QAOA**: Each penalty term gets its own linear-ramp schedule γ_i(t) = α_i · t, with α_i as variational parameter. Eliminates nested penalty tuning, scales efficiently to multiple constraints.
- **Piecewise-ramp QAOA**: Replace linear ramps with two-segment piecewise schedules. Small parameter overhead (independent of circuit depth) but consistently outperforms lr-QAOA and Lambda-lr-QAOA.
- **Filtered loss variant**: Single hyperparameter tunes the feasibility-optimality trade-off.
- **Benchmarking**: Earth-observation satellite mission planning (budget-constrained MWIS). Both methods exhibit high feasibility rates crucial for industrial applications.
- See `qaoa-feasibility-penalty-scheduling` skill.
...
### MPE-Adam: Multi-Population Evolutionary + Adam Hybrid Optimizer (arxiv:2606.26670)

A two-stage classical optimizer for QAOA parameters that separates global exploration from local convergence:

- **Stage 1**: Multi-population evolutionary search — multiple populations with different diversity strategies explore the parameter landscape broadly, avoiding premature convergence
- **Stage 2**: Adam gradient refinement — best candidates from Stage 1 are fine-tuned with Adam for local convergence
- **Validated on**: MaxCut instances up to 22 nodes (3-regular graphs), achieving higher approximation ratios and lower variance than evolutionary-only and SPSA baselines
- **Implementation pattern**: Modular two-stage pipeline — evolutionary optimizer outputs initial parameters, Adam takes over for refinement
- **Advantages over single-stage**: Complementary strategies (global+local), lower result variance, more robust to local minima
