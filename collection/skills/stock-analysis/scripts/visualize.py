...
class StockVisualizer:
    ...
    def plot_kline_with_ma(self, df: pd.DataFrame, output_path: str):
        ...
        print("📊 Generating K-line with MA chart...")
        ...
        fig, (ax1, ax2) = plt.subplots(
            2,
            1,
            figsize=(self.width / 100, self.height / 100 * 2),
        ...
        for i in range(len(df)):
            ...
            # Draw candlestick
            ax1.vlines(
                x=date, ymin=low_price, ymax=high_price, colors=color, linewidth=1
            )
            ax1.bar(
                x=date,
        ...
        plt.tight_layout()
        plt.savefig(output_path, facecolor=self.bg_color, dpi=150, bbox_inches="tight")
        plt.close()
    ...
    def plot_macd(self, df: pd.DataFrame, output_path: str):
        ...
        # Plot MACD lines
        ax.plot(df["date"], df["MACD_DIF"], label="DIF", color="#ff6b6b", linewidth=1.5)
        ax.plot(df["date"], df["MACD_DEA"], label="DEA", color="#4ecdc4", linewidth=1.5)

        # Plot histogram
        colors = [self.up_color if x >= 0 else self.down_color for x in df["MACD_BAR"]]
    ...
    def plot_kdj(self, df: pd.DataFrame, output_path: str):
        ...
        # Plot KDJ lines
        ax.plot(df["date"], df["KDJ_K"], label="K", color="#f1c40f", linewidth=1.5)
        ax.plot(df["date"], df["KDJ_D"], label="D", color="#3498db", linewidth=1.5)
        ax.plot(df["date"], df["KDJ_J"], label="J", color="#e74c3c", linewidth=1.5)
        ...
        ax.axhline(
            y=20, color=self.up_color, linestyle="--", alpha=0.3, label="Oversold (20)"
        )
    ...
    def plot_rsi(self, df: pd.DataFrame, output_path: str):
        ...
        ax.plot(df["date"], df["RSI12"], label="RSI12", color="#3498db", linewidth=1.5)
        ax.plot(
            df["date"],
            df["RSI24"],
            label="RSI24",
        ...
        ax.axhline(
            y=30, color=self.up_color, linestyle="--", alpha=0.3, label="Oversold (30)"
        )
    ...
    def plot_boll(self, df: pd.DataFrame, output_path: str):
        ...
        ax1.set_ylabel("Price", color=self.text_color)
        ax1.set_title("K-Line with Bollinger Bands (20, 2)", color=self.text_color)
        ax1.legend(loc="upper left")
        ax1.grid(True, color=self.grid_color, alpha=0.3)
        ax1.tick_params(colors=self.text_color)
    ...
    def plot_composite(self, df: pd.DataFrame, output_path: str):
        ...
        ax1.plot(df["date"], df["MA20"], label="MA20", color="#f1c40f", linewidth=1.5)
        ax1.plot(df["date"], df["MA60"], label="MA60", color="#3498db", linewidth=1.5)
        ...
        # Plot MACD
        ax2.plot(
            df["date"], df["MACD_DIF"], label="DIF", color="#ff6b6b", linewidth=1.5
        )
        ax2.plot(
            df["date"], df["MACD_DEA"], label="DEA", color="#4ecdc4", linewidth=1.5
        )
        ...
        # Plot KDJ
        ax3.plot(df["date"], df["KDJ_K"], label="K", color="#f1c40f", linewidth=1.5)
        ax3.plot(df["date"], df["KDJ_J"], label="J", color="#e74c3c", linewidth=1.5)
        ...
        # Plot RSI
        ax4.plot(df["date"], df["RSI12"], label="RSI12", color="#3498db", linewidth=1.5)
...
def main():
    ...
    parser.add_argument(
        "--width", type=int, default=1200, help="Chart width in pixels (default: 1200)"
    )

    parser.add_argument(
        "--height", type=int, default=600, help="Chart height in pixels (default: 600)"
    )
    ...
    # Initialize visualizer
    visualizer = StockVisualizer(theme=args.theme, width=args.width, height=args.height)
