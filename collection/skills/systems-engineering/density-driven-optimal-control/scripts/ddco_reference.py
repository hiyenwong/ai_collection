...
class DensityDrivenOptimalControl:
    ...
    def compute_control(
        self, states: np.ndarray, target_density: Callable, time: float = 0.0
    ) -> np.ndarray:
        """
    ...
    def _density_to_positions(
        self, target_density: Callable, n_samples: int = 10000
    ) -> np.ndarray:
        """
        ...
        indices = np.random.choice(
            len(samples), size=self.n_agents, replace=False, p=weights
        )
    ...
    def simulate(
        ...
        for t in range(n_steps):
            ...
            states = (
                states
                + (self.A @ states.T + self.B @ controls.T).T * self.dt
                + noise * np.sqrt(self.dt)
            )
    ...
    def compute_density_mismatch(
        self, states: np.ndarray, target_density: Callable, grid_resolution: int = 50
    ) -> float:
        """
    ...
    for idx, (ax, t, title) in enumerate(zip(axes.flat, time_points, titles)):
        ...
        if t > 0:
            for i in range(states.shape[0]):
                ...
                ax.plot(
                    trajectory[:, 0], trajectory[:, 1], "b-", alpha=0.3, linewidth=0.5
                )
...
def main():
    ...
    states_history, controls_history = ddco.simulate(
        initial_states, target_density, args.time, noise_std=args.noise
    )
    ...
    print(
        f"  Average control effort: {np.mean(np.linalg.norm(controls_history, axis=2)):.4f}"
    )
