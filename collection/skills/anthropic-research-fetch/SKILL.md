...
- **web_extract is BLOCKED for anthropic.com article URLs** (returns "Blocked: URL targets a private or internal network address" — a false positive from the security scanner). The fetch script avoids this by downloading the HTML directly with urllib and using regex extraction (see `/Users/hiyenwong/.openclaw/workspace/scripts/fetch_anthropic_research.py`). Do NOT use web_extract on anthropic.com URLs; for article bodies after getting the URL list, use `browser_navigate(<url>)` + `browser_snapshot(full=true)` to read article content. This is the single most common time-sink in this pipeline — go straight to the browser.
