...
def chsh_test(E_ab: float, E_ab_p: float, E_ap_b: float, E_ap_bp: float) -> Tuple[float, bool]:
...
def order_effect_noncommutativity(P_A: np.ndarray, P_B: np.ndarray, psi: np.ndarray) -> Tuple[float, float]:
