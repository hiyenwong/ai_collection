...
class ComputationalLesion:
    ...
    def identify_important_parameters(
        self, language: str = None, importance_threshold: float = 0.9
    ...
    def create_language_specific_lesion(
        self, language: str, zeroing_fraction: float = 0.1
    ) -> nn.Module:
        """
...
class BrainEncoder:
    ...
    def extract_embeddings(
        self, model: nn.Module, texts: List[str], layer: int = -2
    ) -> np.ndarray:
        """
...
class LesionExperiment:
    ...
    def run_lesion_study(
        ...
        Returns:
            Results dictionary
        """
        results = {"intact": {}, "shared_lesion": {}, "language_lesions": {}}

        # 1. Baseline with intact model
        print("Testing intact model...")
        for lang in self.languages:
            embeddings = encoder.extract_embeddings(self.lesion.model, texts[lang])
    ...
    def compute_lesion_effects(self) -> Dict:
        ...
        """
        effects = {"shared_core_reduction": {}, "language_specificity": {}}
        ...
        for lesion_lang in self.languages:
            ...
            for test_lang in self.languages:
                ...
                lesioned_corr = self.results["language_lesions"][lesion_lang][
                    test_lang
                ]["mean_correlation"]
                reduction = (intact_corr - lesioned_corr) / intact_corr
