...
6. **Use browser navigation as primary fallback**: When programmatic HTTP clients fail due to proxy/SSL issues (common in cron jobs or restricted environments), use `browser_navigate` with HTTPS URLs to access arXiv pages directly. This approach:
   - Bypasses SSL certificate verification issues
   - Works reliably through proxies
   - Provides full page content including metadata and abstracts
   - Is particularly effective for accessing individual paper pages and RSS feeds
7. **Leverage category-specific RSS feeds via browser**: For neuroscience research, navigate directly to `https://rss.arxiv.org/rss/q-bio.NC` and `https://rss.arxiv.org/rss/cs.NE` using browser tools to get targeted, recent papers without complex API parsing.
