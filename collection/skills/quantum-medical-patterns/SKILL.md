...
description: "Reusable research patterns from quantum ML in healthcare: hybrid modeling, entanglement PET (J-PET), CV photonic QNNs, quantum autoencoders, quantum ophthalmology, generative models, kernel methods, clinical trials, tensor-network federated learning, TSHF fusion. 4 sub-class skills exist. Triggers: quantum medical, entanglement PET, CV-QNN, quantum autoencoder, quantum ophthalmology, J-PET, 量子医疗."
---
...
### Sub-class taxonomy reference
For detailed sub-class classification and decision rules, see [references/quantum-medical-taxonomy.md](references/quantum-medical-taxonomy.md).

**Before creating new medicine+quantum skills**: Check against the 4 established sub-classes above. New papers should extend existing patterns or add genuinely new sub-classes only.

### Pattern 11: Entanglement-Based PET Imaging (J-PET)
**Core idea**: Exploit quantum entanglement of polarization between annihilation photons as an additional diagnostic channel beyond standard PET uptake.
- Plastic scintillators detect via Compton scattering → simultaneous position + time + polarization
- Entanglement degree = correlation of polarization planes between coincidence pairs
- First in vivo human imaging (2606.29421): liver/spleen entanglement between maximally entangled and separable
- PET with entangled Compton events (2606.25804): 20% higher SBR, up to 10% sensitivity increase

### Pattern 12: Parameter-Efficient CV Photonic QNNs for Edge Medical AI
**Core idea**: Room-temperature photonic quantum computing for edge medical classification with minimal parameters.
- Simplified Phi-D-U1 layer cuts parameters 40-45% vs full Killoran layer
- 18 parameters achieve 100% calibrated accuracy for oral cancer detection (2606.28252)
- Mitigates barren plateaus: gradient variance improved ~58 orders of magnitude
- MobileNetV1 + PCA 16-dim + CV-QNN pipeline

### Pattern 13: Compression-Driven Anomaly Detection via Quantum Autoencoders
**Core idea**: QAEs trained on normal data detect anomalies via incompressibility.
- Angle encoding + variational encoder-decoder with trash qubits (2606.27411)
- ROC-AUC ~0.95 slice-level, ~0.813 patch-level on brain MRI
- Encoder-decoder asymmetry: detection from structured compression, not decoder expressivity
- Spatially localized anomaly heatmaps aligned with tumorous regions

### Pattern 14: Quantum Ophthalmology
**Core idea**: Four complementary quantum approaches to ophthalmic imaging (2606.19238):
1. Photon-limited retinal imaging (OCT + single-photon detection)
2. Correlation-based imaging (ghost imaging for low-light/scattering)
3. Nanoscale optical probes (quantum dots for contrast/targeting)
4. Quantum-limited visual perception (single-photon level structured light)
