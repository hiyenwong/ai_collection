...
- **kg_relations schema**: Columns are `(source, target, type, weight)` — NOT `source_id`, `target_id`, `relation_type`. **Verified 2026-06-06**: Using `source_id`/`target_id` in queries causes `OperationalError: no such column`. Always use `source`/`target`/`type`.
