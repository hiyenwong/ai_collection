...
### patch Tool Limitation for Multi-Line INDEX.md Inserts (2026-06-15)

The `patch` tool (mode='patch') fails with "File name too long" when passed multi-line V4A patch content for INDEX.md insertions. The parser seems to treat the entire patch content as a filename. **Do NOT use `patch` mode for INDEX.md multi-line insertions.**

**Working alternatives:**
1. **Python string split-and-insert** (most reliable for large inserts): Read file as lines, find insertion point, splice in new content, write back. See `Insertion via Python` section above.
2. **patch tool in replace mode** (mode='replace'): Find a unique marker line in INDEX.md and replace it with marker + new content. Works but requires a unique enough marker string.
3. Always verify insertion with `grep` after writing — sibling sessions may modify INDEX.md between your read and write.
