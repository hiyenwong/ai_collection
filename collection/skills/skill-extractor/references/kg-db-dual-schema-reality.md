...
**CRITICAL (2026-06-06 confirmed)**: `kg_entities.id` is INTEGER auto-increment, NOT TEXT. Do NOT pass arxiv IDs as the `id` column value. Omit `id` from INSERT and use `cur.lastrowid`. Duplicate check via `WHERE title LIKE '%{arxiv_id}%'` or `WHERE url = 'https://arxiv.org/abs/{id}'`.
