...
KG_DB_PATH = "/Users/hiyenwong/wiki/kg.db"
...
def get_paper(conn: sqlite3.Connection, paper_id: int) -> Optional[Dict]:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, name, properties FROM kg_entities WHERE id = ?", (paper_id,)
    )
    row = cursor.fetchone()
    if not row:
        return None
...
def main():
    ...
    parser.add_argument(
        "--kg", type=str, default=KG_DB_PATH, help="Knowledge graph database path"
    )
