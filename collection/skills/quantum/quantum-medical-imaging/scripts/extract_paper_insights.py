...
def extract_paper_insights(text: str) -> dict:
    ...
    for match in metrics_found:
        insights["metrics"].append({"value": int(match[0]), "type": match[1]})
    ...
    for sentence in sentences:
        if any(
            ...
            for kw in ["demonstrate", "show", "achieve", "improve"]
        ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract insights from quantum medical imaging paper"
    )
