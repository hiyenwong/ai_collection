...
class QNASOptimizer:
    ...
    def __init__(
        self, search_space: Dict, population_size: int = 100, generations: int = 50
    ):
    ...
    def _generate_architecture(self) -> Dict:
        ...
        return {
            ...
            "depth": random.randint(
                self.search_space["depth"][0], self.search_space["depth"][-1]
            ),
            "qubits": random.randint(
                self.search_space["qubits"][0], self.search_space["qubits"][-1]
            ),
        }
    ...
    def _crossover(self, ind1: Dict, ind2: Dict) -> Tuple[Dict, Dict]:
        ...
        # Swap CNOT pattern
        if random.random() < 0.5:
            ind1["cnot_pattern"], ind2["cnot_pattern"] = (
                ind2["cnot_pattern"],
                ind1["cnot_pattern"],
            )
    ...
    def _mutate(self, architecture: Dict) -> Dict:
        ...
        # Mutate CNOT pattern
        if random.random() < 0.2:
            architecture["cnot_pattern"] = random.choice(
                self.search_space["cnot_pattern"]
            )
    ...
    def optimize(self, evaluate_fn: Callable, verbose: bool = True) -> List[Dict]:
        """
        ...
        for gen in range(self.generations):
            ...
            if verbose and gen % 10 == 0:
                pareto_front = tools.sortNondominated(population, self.population_size)[
                    0
                ]
    ...
    def analyze_pareto_front(self, pareto_front: List[Dict]) -> Dict:
        ...
        """
        analysis = {"size": len(pareto_front)}
...
if __name__ == "__main__":
    ...
    optimizer = QNASOptimizer(
        search_space=search_space, population_size=50, generations=20
    )
    ...
    def evaluate(architecture):
        ...
        cutting_overhead = (
            1 if architecture["qubits"] <= 8 else 4 ** (architecture["qubits"] - 8)
        )
