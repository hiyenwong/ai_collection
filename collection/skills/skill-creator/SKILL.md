...
- **KG update requires schema verification first (2026-06-05 confirmed)**: Scripts that import papers into kg.db must verify the actual table schema BEFORE writing INSERT statements. Run `sqlite3 ~/.hermes/kg.db ".schema"` or `PRAGMA table_info(table_name)` for each target table. **Pitfall**: Assuming column names from documentation without verification causes errors like "table relations has no column named source_id". **Fix**: Check actual schema → identify correct column names (e.g., Hermes kg.db relations uses `from_entity/to_entity/relationship_type`, NOT `source_id/target_id`; entities uses `type` column, NOT `entity_type`) → write INSERT with verified columns. **Reference**: See `knowledge-graph-ops` skill for complete schema with column types (NOT "kg-db-operations").
