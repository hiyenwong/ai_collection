...
## ⚠️ Critical Warning: The Inverse Born Rule Fallacy

Amplitude encoding via the mapping `ψ = √P` (loading classical probability distributions as quantum amplitudes) causes **phase-deafness** — the data is restricted to the positive real orthant S⁺, effectively abelianizing the Hilbert space and stripping away relative phases that constitute the true computational resource for quantum advantage.

**Detect it**: `psi = np.sqrt(probability_distribution)` → phase-deaf ❌
**Safe**: `psi_i ∝ x_i` where x carries sign/phase information → OK ✅

This affects QML classification/learning tasks (not Monte Carlo integration). **Remedy**: Use Dynamical Hamiltonian Encoding (QIFT) — treat data as generators of quantum evolution via Suzuki-Trotter sandwich. See `inverse-born-rule-fallacy` skill for details.
