...
description: >
  ...
  verification of Weber's conjecture, Principal Ideal Problem solvability, Structured CVP
  Distance on the Log-Unit Lattice, Ring-LWE and Module-LWE security reductions, and
  cyclotomic field arithmetic. Combines computational number theory (Fukuda-Komatsu sieve,
  Herbrand's theorem, Coarse Lattice Theorem, Trigamma Theorem) with lattice-based
  cryptographic security analysis. Activation: module lattice security, Weber conjecture,
  Ring-LWE, Module-LWE, post-quantum cryptography, lattice cryptography, CVP distance,
  log-unit lattice, 模格安全, log-unit lattice, CVP距离.
---
...
## Part III: Structured CVP Distance on the Log-Unit Lattice

**Source Paper**: Ming-Xing Luo. "Module Lattice Security (Part III): Structured CVP Distance
on the Log-Unit Lattice" (arXiv: 2605.17404)

### Core Results

1. **L² CVP Distance Convergence**: The L² CVP distance from a random short ring element to
   the log-unit lattice of Q(ζ_{2^k}) converges to π/(2√6)·√n as n = 2^{k-1} → ∞.

2. **Voronoi Cell Containment**: The target lies inside the Voronoi cell of the origin for
   k ≥ 4, implying Babai's algorithm returns zero for structured targets.

3. **L^∞ Norm Analysis**: Maximum over n sub-Gaussian coordinates yields O(√log n)
   sub-polynomial approximation factor for the Short Generator Problem.

4. **Coarse Lattice Theorem**: Babai's algorithm returns zero for all structured targets,
   yet exactly recovers unit perturbations of arbitrary size.

5. **Trigamma Theorem**: For module determinant ideals, proves an intrinsic imbalance
   σ_{g₀} = O(1) independent of the modulus q.

6. **CDPR Factor Reduction**: Combined with Parts I and II, reduces the CDPR factor for
   ML-KEM from exp(Õ(√n)) to a sub-polynomial value.

### Cryptographic Implications

| Result | Impact on PQC |
|--------|--------------|
| Voronoi cell containment | Babai's algorithm efficient for structured instances |
| Trigamma Theorem | O(1) imbalance → tighter security bounds |
| Sub-polynomial CDPR | ML-KEM security tighter than exponential estimates |

### Integration with Parts I-II

Part I verified Weber's conjecture (class group structure). Part III leverages this to
analyze the geometric structure of the log-unit lattice, showing that the lattice geometry
itself provides security guarantees beyond what class group analysis alone reveals.
