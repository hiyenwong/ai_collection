...
- **Hardware gap (2026-07-07 NEW)**: On real hardware (ibm_kingston, Heron r2), XY-mixer simulator-hardware energy gap stays within |0.8| across all depths — but penalty-based variants diverge up to |53.9|. The structural guarantee partially breaks under gate-level noise, so honest hardware accounting is essential.

## Real-Hardware Validation (COMET, arXiv: 2607.02622)

First real-hardware comparison of XY-mixer vs penalty QAOA on 12-qubit CRISPR gene editing instance:
- XY-mixer: >95% optimum probability at p=3; hardware gap |ΔE| ≤ 0.8
- Penalty variants: <6% optimum at every depth; hardware gap up to |ΔE| = 53.9
- No penalty coefficient tuning needed with XY-mixer — enforced structurally
- Related umbrella skill: `comet-constraint-preserving-qaoa`
