...
### Hamiltonian-Informed QAS Benchmarking (HamQASBench, arXiv: 2607.04845)

When evaluating QAS methods, energy accuracy alone misses critical structural failures:

| Failure Mode | Detection Method |
|---|---|
| Over-parameterization | Critical-structure extraction vs minimal circuit size |
| Eigenstate commitment (degeneracy) | Pairwise state fidelity |
| Representation bottleneck (strong correlation) | Per-qubit entanglement analysis |
| Topology-induced routing failure | Circuit topology vs Hamiltonian structure match |

**Workflow**: Organize problem instances into structural tiers via Pauli operator fingerprints + computational basis representation + ground-state entanglement → run QAS per tier → apply diagnostics beyond energy metrics.
