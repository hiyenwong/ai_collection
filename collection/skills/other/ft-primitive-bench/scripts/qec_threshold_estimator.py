...
def analyze_primitive_sensitivity(
    primitive: str, noise_model: dict, code_distance: int = 5
) -> dict:
    """
    ...
    multipliers = {
        ...
        "phase": 2.0
        + 0.5 * (pm / px)
        + 0.2 * abs(np.log10(bias)),  # Combined sensitivity
    }
...
def main():
    ...
    parser.add_argument(
        "--bias", type=float, default=1.0, help="Noise bias ratio (pZ/pX)"
    )
    parser.add_argument(
        "--code_distances",
        type=int,
