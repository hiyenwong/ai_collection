...
- `read` - Read neural recording data and research papers
- `write` - Save manifold analysis results and visualizations
- `exec` - Run Python scripts for dimensionality reduction and manifold analysis

## Instructions for Agents

Follow these steps when helping users analyze neural manifold dynamics:

1. **Identify the analysis goal**: Determine if the user needs jPCA (rotational dynamics), dPCA (demixed components), or GPFA (smooth trajectories)
2. **Prepare the data**: Format neural recordings as population activity matrices
3. **Apply dimensionality reduction**: Use the appropriate method from this skill
4. **Interpret results**: Analyze trajectories, variance explained, and behavioral correlations

## Examples

### Example 1: jPCA Rotational Dynamics

```
User: "分析运动皮层神经群的旋转动力学"

Execute:
1. Load neural population activity during reaching
2. Apply jPCA to find rotational dynamics
3. Visualize trajectories in the jPC plane
4. Interpret rotation frequency and behavioral correlation
```

### Example 2: dPCA Demixed Analysis

```
User: "用dPCA解混条件相关和时间相关的神经活动"

Execute:
1. Load neural data with experimental conditions
2. Apply dPCA to demix variance components
3. Visualize condition-specific and time-specific manifolds
4. Quantify variance explained by each component
```
