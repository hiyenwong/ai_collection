...
class SuperCircuit:
    ...
    def __init__(
        ...
        self.shared_weights = np.random.uniform(
            -np.pi, np.pi, size=(max_depth, max_qubits, 3)
        )
    ...
    def build_circuit(self, architecture: Dict, features: np.ndarray):
        """
        ...
        def circuit(weights):
            ...
            for layer in range(architecture["depth"]):
                ...
                # Variational layer
                self.variational_layer(architecture["qubits"], layer, weights)
    ...
    def train(self, dataset: Dict, epochs: int = 50, learning_rate: float = 0.01):
        """
