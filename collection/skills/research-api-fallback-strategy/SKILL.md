...
- **kg_entities schema quirks (as of May 2026)** — TWO kg.db files with DIFFERENT schemas exist on this system. See [references/kgdb-two-schemas.md](references/kgdb-two-schemas.md). 
...
1. RSS Feed (`curl` via terminal) — primary discovery
2. KG.db (`sqlite3` via terminal) — primary fallback with 1500+ entities
3. Workspace JSON caches — zero-API fallback
4. kg_tool (pagerank, stats) via terminal — graph analysis
5. Direct `write_file` to `~/.hermes/skills/{name}/SKILL.md` — skill creation (no execute_code)
