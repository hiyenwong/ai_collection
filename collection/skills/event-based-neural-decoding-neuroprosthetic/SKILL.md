---
name: event-based-neural-decoding-neuroprosthetic
description: >-
  Event-based neural decoding framework using spiking GRU with sparse graded
  spikes for efficient on-device motor control. Achieves >90% decoding accuracy
  with <1mW power consumption on neuromorphic hardware for neuroprosthetic
  applications.
---
...
- See `templates/event_based_gru_template.py` for a complete PyTorch implementation template.

## Usage Hint
Integrate this decoding pipeline into a brain-computer interface (BCI) pipeline where neural events are streamed from an implantable sensor to an on-device decoder, enabling low-latency, low-power prosthetic control. See `templates/event_based_gru_template.py` for a starting implementation.