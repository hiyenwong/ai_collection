...
### 7. Sector Coherence Law for Noisy Equivariant QNNs (2026 — arXiv:2606.30688)

**Core Insight**: Symmetry provides structure but doesn't keep equivariant QNNs trainable under noise. The key quantity is **sector coherence rate** — a Rayleigh quotient of the noise generator along the gradient-carrying mode.

**Two-Effect Framework**:
1. **Causality effect**: Fixes WHERE the gradient lives (backward light cone within charge sector)
2. **Coherence effect**: Determines HOW FAST it decays (contraction of off-diagonal sector modes)

**Key Results**:
- Light-cone reduction pins noiseless gradient to sector-restricted cone; lower bound independent of total qubit count
- Sector coherence rate = Rayleigh quotient `<ψ|L|ψ> / <ψ|ψ>` of noise generator L along gradient direction
- **Critical test**: Correlated dephasing has large worst-case rate but near-zero aligned rate → training law predicts no gradient loss, confirmed by simulation (R²=0.979)
- Sector coherence rate outperforms ALL standard channel diagnostics

**Why this matters for barren plateaus**: Standard noise metrics (diamond norm, worst-case rate) don't predict gradient survival under decoherence. The coherence law provides a precise, computable criterion for trainability — enabling noise-shaping that steers decoherence into sector-invisible directions.

**Workflow**: (1) Identify conserved charge sector of U(1)-equivariant circuit, (2) Map noise channel to Lindbladian generator L, (3) Compute aligned coherence rate, (4) rate ≈ 0 → gradient preserved; rate >> 0 → exponential decay with depth, (5) Consider correlated noise with better aligned rates

**Pitfall**: Derived for U(1)-equivariant brickwork circuits only; assumes weak noise (ε << 1). Other symmetries need separate analysis.
...
| **Equivariant/symmetric QNN under noise** | **Sector coherence law (2606.30688): compute Rayleigh quotient of noise generator along gradient mode** ← NEW |
...
- `qml-expressivity-trainability`: QML expressivity and trainability analysis
