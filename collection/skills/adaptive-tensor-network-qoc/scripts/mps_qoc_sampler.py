...
class MPSSampler:
    ...
    def sample(self, n_samples: int = 1) -> np.ndarray:
        ...
        for _ in range(n_samples):
            ...
            for i in range(self.n_sites):
                ...
                d_right_i = tensor_i.shape[2]
                
                # Ensure left_vec has correct shape
                if left_vec.shape[0] != d_left_i:
                    # Pad or truncate
                    new_vec = np.zeros(d_left_i)
                    new_vec[:min(len(left_vec), d_left_i)] = left_vec[:min(len(left_vec), d_left_i)]
                    left_vec = new_vec
    ...
    def update(self, sequences: np.ndarray, scores: np.ndarray, 
               learning_rate: float = 0.01, top_fraction: float = 0.2):
        """
...
def run_qoc_optimization(
    ...
    for iteration in range(n_iterations):
        ...
        if verbose and (iteration % 10 == 0 or iteration == n_iterations - 1):
            print(f"Iteration {iteration:3d} | Best score: {best_score:.4f} | "
