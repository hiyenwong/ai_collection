...
def extract_expert_domain(content: str) -> str:
    ...
    if lines:
        ...
        match = re.match(r'You are an? (?:expert|senior) (?:in|developer|programmer)?\s*(.+?)(?:\.|,|$)', first_line, re.IGNORECASE)
        if match:
...
def extract_tasks(content: str) -> list[str]:
    ...
    for section in task_sections:
        ...
        if any(word in section_lower for word in ['test', 'debug', 'build', 'deploy', 'security', 'performance']):
...
def main():
    ...
    parser.add_argument("--batch", help="Batch import a category (e.g., typescript, react)")
