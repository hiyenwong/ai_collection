...
def get_recommendations(expressivity: float, risk: str) -> List[str]:
    ...
    if expressivity < 0.1:
        recommendations.append("Increase circuit depth or add more entangling gates to improve expressivity")
    
    if risk == 'HIGH':
        recommendations.append("Reduce circuit depth to 5-10 layers")
        recommendations.append("Use local cost functions instead of global measurements")
        recommendations.append("Add problem-specific structure to reduce parameter space")
    ...
    if not recommendations:
        recommendations.append("Circuit design is good. Proceed with standard training.")
...
def main():
    ...
    print("=" * 60)
    print()
    print(f"Circuit: {circuit_spec.get('n_qubits', 4)} qubits, {circuit_spec.get('depth', 3)} layers")
    print()
    ...
    print(f"Gradient variance estimate: {result['barren_plateau']['variance_estimate']:.2e}")
