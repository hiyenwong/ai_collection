...
class ConceptProber:
    ...
    def extract_hidden_states(self, texts: List[str], layer: int) -> torch.Tensor:
        """
    ...
    def train_probe(
        self, texts: List[str], labels: List[int], layer: int
...
class NeuronIntervener:
    ...
    def _make_hook(self, neuron_idx: int, direction: str, strength: float):
        """Create hook function for neuron intervention."""

        def hook_fn(module, input, output):
            modified = output.clone()
            if direction == "enhance":
                modified[:, :, neuron_idx] += strength * torch.abs(
                    modified[:, :, neuron_idx]
                )
            else:  # suppress
    ...
    def intervene(
        self, text: str, trait: str, direction: str = "enhance", strength: float = 0.5
    ) -> str:
        """
