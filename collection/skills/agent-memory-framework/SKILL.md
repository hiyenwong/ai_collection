...
- `read`: Load memory configuration and conversation history
- `write`: Create memory entries and update memory store
- `exec`: Run memory extraction and retrieval scripts
- `memory_search`: Search existing memories for context

## Instructions for Agents

### Step 1: Extract Information
Analyze the current conversation to extract key facts, preferences, and context that should be stored in memory.

### Step 2: Decide Update Action
Use RL policy to determine: ADD (new memory), UPDATE (existing memory), DELETE (outdated memory), or NOOP (no change).

### Step 3: Store Memory
Save the extracted information to the appropriate memory store (vector DB, knowledge graph, or structured database).

### Step 4: Retrieve Relevant Memories
When responding to user queries, search memory store for relevant past information to include in context.

### Step 5: Generate Response
Use retrieved memories to provide personalized, context-aware responses.
...
```
User: "I prefer Python for data analysis and dislike JavaScript"

Agent:
1. Extract: User prefers Python, dislikes JavaScript for data analysis
2. Decide: ADD to user preferences
3. Store: {"preference": "Python for data analysis", "confidence": 0.95}
4. Later query: "What language should I use for data tasks?"
5. Retrieve: "Based on your preferences, I recommend Python..."
```

### Example 2: Project Context Memory

```
User: "We're building a RAG system using LanceDB"

Agent:
1. Extract: Project = RAG system, Technology = LanceDB
2. Decide: ADD to project context
3. Store: {"project": "RAG system", "tech_stack": ["LanceDB"]}
4. Later query: "How should we handle embeddings?"
5. Retrieve: "For your RAG system with LanceDB, consider..."
```
