...
class ProjectArchitecture:
    ...
    layers: list = field(
        default_factory=lambda: ["types", "config", "repo", "service", "runtime", "ui"]
    )
    cross_cutting: list = field(
        default_factory=lambda: ["auth", "logging", "feature-flags"]
    )
...
class InterviewConductor:
    ...
    def conduct_phase1_vision(self) -> None:
        ...
        self.config.vision.description = input(
    ...
    def conduct_phase2_architecture(self) -> None:
        ...
        self.config.architecture.domains = [
        ...
        if layers_input:
            self.config.architecture.layers = [
                layer.strip() for layer in layers_input.split(",")
            ]

        print("\n🔀 Cross-cutting concerns? (default: auth, logging, feature-flags)")
        cc_input = input("> ").strip()
        if cc_input:
            self.config.architecture.cross_cutting = [
                c.strip() for c in cc_input.split(",")
            ]
    ...
    def conduct_phase3_agent_config(self) -> None:
        ...
        if not use_defaults:
            ...
            self.config.agent_config.conventions["max_file_size"] = int(
                input("> ").strip() or "300"
            )
        ...
        self.config.agent_config.constraints.append(
            f"test_coverage >= {coverage or '80'}%"
        )
...
"""
    ...
    def _generate_domain_table(self) -> str:
        ...
        for domain in self.config.architecture.domains:
            lines.append(
                f"| {domain} | {self.config.docs_path}design/{domain.lower()}.md | Active |"
            )
        return "\n".join(lines)
...
"""
    ...
    def generate_docs_structure(self) -> list:
        ...
        (docs_path / "architecture" / "README.md").write_text(
            self._architecture_readme()
        )
    ...
    def _design_index(self) -> str:
        domains_list = "\n".join(
            [f"- [{d}]({d.lower()}.md)" for d in self.config.architecture.domains]
        )
...
"""
    ...
    def _architecture_readme(self) -> str:
        domains_table = "\n".join(
            [f"| {d} | Domain | All |" for d in self.config.architecture.domains]
        )
...
"""
    ...
    def _quality_domains(self) -> str:
        rows = "\n".join(
            [f"| {d} | 🟡 Pending | - | - |" for d in self.config.architecture.domains]
        )
    ...
    def validate(self) -> int:
        ...
        if agents_md.exists():
            ...
            if lines < 200:
                ...
                self.checks.append(
                    f"⚠ AGENTS.md too long ({lines} lines, should be <200)"
                )
        else:
