...
- `read` - Read SNN papers, research notes, and configuration files
- `write` - Save training configurations and analysis results
- `exec` - Run Python training scripts for surrogate gradient experiments
- `bash` - Execute training pipelines and evaluate SNN models

## Instructions for Agents

Follow these steps when helping users with surrogate gradient SNN training:

1. **Understand the user's SNN architecture**: Determine neuron type, network depth, and temporal requirements
2. **Select surrogate gradient function**: Recommend the appropriate gradient function based on the use case (fast sigmoid for general use, arctan for fine control)
3. **Configure training pipeline**: Set up temporal processing layers with batch normalization
4. **Provide implementation code**: Use the complete neuron and layer implementations from this skill
5. **Troubleshoot training issues**: Check for vanishing/exploding gradients, suggest adaptive surrogates

## Examples

### Example 1: Basic Surrogate Gradient SNN

```
User: "How do I train a spiking neural network with backpropagation?"

Execute:
1. Explain the surrogate gradient approach
2. Provide the FastSpikeNeuron implementation
3. Show the SnnTemporalLayer wrapper
4. Demonstrate the training pipeline
```

### Example 2: Choosing a Surrogate Gradient

```
User: "Which surrogate gradient function should I use?"

Execute:
1. Present the comparison table from Performance Comparison
2. Recommend fast sigmoid for most use cases
3. Suggest arctan for fine-grained control
4. Recommend adaptive surrogates for deep networks
```
