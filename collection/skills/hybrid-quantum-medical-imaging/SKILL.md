...
## Quantum Autoencoder for Medical Imaging (New Pattern)

### Compression-Driven Anomaly Detection
**QAE Architecture**: Angle encoding → variational encoder-decoder with trash qubits → reconstruction
- **Key Insight**: Structured encoder compression (not decoder expressivity) drives anomaly detection
- **Performance**: Slice-level ROC-AUC ~0.95, patch-level ~0.813 on brain MRI
- **Advantage**: Produces spatially localized anomaly heatmaps aligned with tumors
- **Implementation**: Train encoder to discard normal data info to trash qubits; anomalies resist compression

### Use When
- Quantum anomaly detection in medical imaging workflows
- Interpretable ML where spatial localization of anomalies matters
- Compression-based outlier detection with principled threshold selection

## Quantum Entanglement Imaging (New Paradigm)

### First-in-Human Quantum Entanglement PET
**Breakthrough**: First in vivo imaging of quantum entanglement degree from positron-electron annihilation
- **Technology**: J-PET plastic scintillator scanner measures Compton effect for position + time + polarization simultaneously
- **Finding**: Entanglement degree between separable and maximally entangled states varies by tissue (liver vs spleen)
- **Impact**: Opens quantum-correlation-based diagnostic contrast mechanism
- **Complementary Result**: PET with polarization-correlated Compton events achieves 10% sensitivity increase, 20% better signal-to-background
