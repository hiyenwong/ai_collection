...
class MemristiveNeuron:
    ...
    def __init__(
        self, params: MemristorParams = None, R_series: float = 10000, dt: float = 1e-4
    ):
        """
...
def demonstrate_encoding_modes():
    ...
    for idx, I_amp in enumerate(input_strengths):
        ...
        ax.axhline(
            y=neuron.params.Vth_set * 1000, color="r", linestyle="--", label="Vth"
        )
    ...
    for r in results:
        ...
        print(
            f"{r['input'] * 1e6:<12.1f} {ttfs_str:<12} {r['count']:<8} {r['rate']:<10.1f}"
        )
...
def compare_encoding_modes():
    ...
    input_currents = np.array([np.ones(n_steps) * s for s in stimulus])
    ...
    if ttfs_values:
        print("\n1. TTFS Encoding:")
        print(
            f"   Range: {min(ttfs_values) * 1000:.1f} - {max(ttfs_values) * 1000:.1f} ms"
        )
    ...
    ttfs_corr = (
        np.corrcoef(stimulus[: len(ttfs_values)], ttfs_values)[0, 1]
        if len(ttfs_values) > 1
        else 0
    )
