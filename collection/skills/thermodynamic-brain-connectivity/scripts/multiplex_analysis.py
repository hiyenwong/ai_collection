...
class ThermodynamicBrainAnalyzer:
    ...
    def infer_functional_connectivity(
        self, synaptic_connectome: np.ndarray, method: str = "boltzmann"
    ) -> np.ndarray:
        """
    ...
    def build_multiplex_network(
        ...
        coupling = self._compute_layer_coupling(synaptic_functional, extrasynaptic)
    ...
    def _compute_layer_coupling(
        self, synaptic: np.ndarray, extrasynaptic: np.ndarray
    ) -> np.ndarray:
    ...
    def _compute_node_correlation(
        self, connectivity: np.ndarray, node_idx: int
    ) -> float:
    ...
    def _compute_multiplex_metrics(self, multiplex: MultiplexNetwork) -> Dict:
        ...
        return {
            ...
            "layer_coupling": multiplex.coupling[0, 1]
            if multiplex.coupling is not None
            else 0,
        }
