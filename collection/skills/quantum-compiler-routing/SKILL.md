...
## Compilation Patterns Beyond Routing (2026-07-02)

Routing is one axis of quantum compilation. Two other critical optimization axes discovered in recent work:

### Error-Budget-Aware Decomposition Selection
- Instead of minimizing gate count, minimize **two-qubit infidelity** per gate decomposition
- **Critical safety finding**: pattern-matched relative-phase Toffoli substitution is silently incorrect — requires instance-specific equivalence verification
- Up to 39.5% two-qubit gate reduction, 36-43% circuit infidelity reduction (arXiv:2606.31791)
- See skill: `context-verified-toffoli-decomposition`

### Static Buffer-Relay Fabric (Movement Elimination)
- Replace dynamic qubit shuttling entirely with static buffer atoms mediating long-range interactions
- BRIDGE pattern: ~10× fidelity improvement, ~540× lower execution time vs. shuttle-based (arXiv:2606.31833)
- Key: dual-species 2D interleaved array with calibrated heteronuclear Rydberg channels
- See skill: `bridge-buffer-relay-compilation`

### Program Representation for Parallelization
- Permeability DAG data structure captures commutation relations across abstraction levels
- Enables automatic parallelization + memory management + uncomputation synthesis (arXiv:2606.31837)
- Retargetable across NISQ/FTQC and per-device execution speeds
- See skill: `permeability-dag-quantum-parallelization`

## Related Methods

- **bridge-buffer-relay-compilation** -- Eliminates qubit movement via static buffer-relay fabric (arXiv:2606.31833)
- **context-verified-toffoli-decomposition** -- Error-budget-aware Toffoli decomposition with instance-specific equivalence verification (arXiv:2606.31791)
- **permeability-dag-quantum-parallelization** -- Permeability DAG for automatic quantum program parallelization and memory management (arXiv:2606.31837)
- **neutral-atom-circuit-mapping** -- NAQC-specific qubit routing for zoned neutral atom architectures (arXiv:2606.20503). Uses graph-theoretic NIP + GA; broader routing framework; this skill is specialized for atom transfer optimization under zone constraints.
