...
#### Hybrid Classical-Quantum VAE + Quantum Kernel SVM (HCQ Pipeline)
- **Architecture**: Supervised 3D β-VAE for disease-aware feature extraction → PLS dimension reduction → ZZ quantum feature map → precomputed-kernel SVM
- **Key innovation**: Quantum kernel operates on **disease-aware features** learned end-to-end by supervised autoencoder, NOT pre-extracted generic features
- **Three-loss VAE training**: reconstruction + KL + focal classification losses ensure features encode pathology-relevant patterns
- **PLS for qubit matching**: Partial Least Squares selects exactly N components matching available qubits while maximizing class separation
- **Precomputed Gram matrix**: Compute N×N pairwise state overlaps once, then train classical SVM — avoids repeated quantum circuit evaluations during training
- **Best for**: Medical image classification (MRI, CT, PET) with small-to-medium datasets (hundreds of samples)
- **Results**: 72.1% accuracy, 0.799 AUC on Alzheimer's classification (308 ADNI-1 subjects, arXiv: 2606.14194)
- **Generalizable**: Any biomedical imaging modality (CT, PET, ultrasound) with similar classification challenges
...
- **HCQ Alzheimer's**: Supervised β-VAE + ZZ Quantum Kernel SVM (arXiv: 2606.14194) — disease-aware feature extraction + quantum classification, 72.1% accuracy, 0.799 AUC
- **HCQ Pipeline Details**: See `references/hcq-alzheimer-pipeline.md` for full two-stage architecture and implementation notes

## Quantum Image Encoding (Fourier-based Methods)

For large-scale medical imaging (≥1024×1024), gate-efficient encoding is critical:
- **Problem**: Existing methods use ~2x gates compared to pixel count
- **Solution**: Fourier-based encoding reduces gates by ≥4x vs pixel count
- **Compression**: Two additional compression techniques further reduce gates + preprocessing time with negligible quality loss
- **Validated on**: BABA robotic thyroidectomy surgical images (1024×1024)
- **arXiv**: 2505.06471
- **When to use**: Any large medical image requiring quantum encoding (CT, MRI, surgical imaging)
...
## Medical Data Security: Post-Quantum Cryptography

Beyond QML for diagnosis, medical pipelines handling sensitive data (adverse event reports, pharmacovigilance, clinical observations) require PQC migration. See `post-quantum-secure-pharmacovigilance` skill for:
- **ML-KEM (CRYSTALS-Kyber)**: Post-quantum key encapsulation replacing RSA/ECDH
- **ML-DSA (CRYSTALS-Dilithium)**: Post-quantum signatures for report authentication
- **Hybrid TLS**: Transition strategy combining classical + PQC during migration
- **Harvest-now-decrypt-later**: Protection for long-lived sensitive medical data (arXiv: 2606.09412)
...
7. **Security Pipeline**: Even with quantum-enhanced diagnosis, the data pipeline itself needs PQC protection — ML-KEM/ML-DSA for pharmacovigilance systems (FDA/EMA reporting pipelines)
...
- HCQ Alzheimer's classification
- quantum kernel SVM medical
- supervised VAE quantum features