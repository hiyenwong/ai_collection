...
|-------|--------------|----------|
| Free Energy-Entropy Duality for Risk-Sensitive RL Investment | LQG stochastic game reformulation of benchmarked allocation | 2606.20903 |
| Heuristic Portfolio Optimization (HPO) | Information-restricted projection of Markowitz solution | 2606.12612 |
| Higher-Order Portfolio Optimization with QAOA | Skewness, kurtosis terms | 2509.01496 |
| Variational and Annealing-Based Quantum Combinatorial Optimization | QAOA vs QA comparative survey | 2603.19117 |
...
- **Penalty methods**: Add penalty terms to objective function (traditional approach — see PITFALL below)
- **Dicke state ansatz** (arxiv:2606.08504): Structural constraint encoding — mixed Dicke states eliminate penalty terms entirely by encoding Hamming weight constraints into circuit structure. Pure Dicke for equality, mixed for inequality, tensor products for multiple groups. Validated on IBM NISQ with CMA-ES. See `dicke-state-portfolio-qaoa` skill.
- **CQM with hard constraints** (D-Wave): Use `LeapHybridCQMSampler` with `ConstrainedQuadraticModel()` for penalty-free constraint handling on annealers
...
## Related Skills
- `free-energy-rl-investment` - Risk-sensitive RL via free energy-entropy duality for continuous-time benchmarked allocation
- `variational-annealing-quantum-combinatorial` - QAOA vs quantum annealing comparative methodology
- `heuristic-portfolio-optimization` - Heuristic portfolio optimization (economics/)
- `qrl-dynamic-portfolio` - Quantum RL for dynamic portfolio optimization
- `quantum-portfolio-optimization` - QAOA with constraint-preserving mixers
- `dicke-state-portfolio-qaoa` - Dicke state ansatz for constraint-preserving QAOA
- `quantum-resilient-decentralized-ai-economy` - Decentralized AI economy with post-quantum security
- `quantum-market-entanglement` - Quantum game theory for market stabilization

## Resources

- `references/algorithms.md`: Detailed algorithm comparison
- See also: `quantum-optimization/higher-order-quantum-optimization-finance` — HOBO for legally-constrained financial problems (CSA, margin, concentration limits) without QUBO reduction overhead
