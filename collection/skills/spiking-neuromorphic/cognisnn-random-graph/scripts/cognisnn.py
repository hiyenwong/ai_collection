...
class LIFNeuron(nn.Module):
    ...
    def forward(self, current, state):
        ...
        v_decayed = torch.where(
            ref_prev > 0, torch.full_like(v_prev, self.v_rest), v_prev
        )

        # Membrane integration
        dv = (v_decayed - self.v_rest + current) / self.tau
        v_new = (
            v_decayed + dv - self.v_th * state.get("spike", torch.zeros_like(v_prev))
        )
...
class RandomGraphTopology:
    ...
    def _generate_graph(self) -> nx.Graph:
        ...
        G = nx.watts_strogatz_graph(self.n_neurons, k, self.small_world_rewiring)
        return G
...
class CogniSNN(nn.Module):
    ...
    def __init__(
        self, input_size, hidden_size, output_size, n_neurons=1000, connection_prob=0.1
    ):
        super().__init__()
        ...
        # Spiking neurons
        self.neurons = nn.ModuleList([LIFNeuron() for _ in range(n_neurons)])
        ...
        self.residual_blocks = nn.ModuleList(
            [SpikingResidualBlock(n_neurons, n_neurons) for _ in range(3)]
        )
    ...
    def forward(self, x, timesteps=10):
        ...
        for t in range(timesteps):
            ...
            for i, neuron in enumerate(self.neurons):
                spike, states[i] = neuron(total_current[:, i : i + 1], states[i])
    ...
    def identify_key_pathways(self, dataloader, num_batches=10):
        ...
        self.key_pathway_mask = {
            name: (f > threshold).float() for name, f in fisher.items()
        }
    ...
    def apply_kplwf_loss(self, new_loss, replay_loss, lambda_replay=1.0):
        ...
        for name, param in self.named_parameters():
            if new_grads[name] is not None:
                ...
                masked_replay = replay_grads.get(name, 0) * self.key_pathway_mask.get(
                    name, 0
                )
    ...
    def dynamic_growth(self, activity_threshold=0.5, correlation_threshold=0.8):
        ...
        if avg_activity > activity_threshold:
            ...
            new_weights[: self.weights.size(0), : self.weights.size(1)] = (
                self.weights.data
            )
...
class CogniSNNTrainer:
    ...
    def _get_replay_data(self):
        ...
        return (
            torch.randn(32, self.model.input_size).to(self.device),
