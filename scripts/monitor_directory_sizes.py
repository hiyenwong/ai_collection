        ...
        for domain_dir in skills_path.iterdir():
            if domain_dir.is_dir() and not domain_dir.name.startswith('.'):
                ...
                results[domain_dir.name] = {
                    'path': str(domain_dir),
                    'count': file_count,
                    'status': 'OK' if file_count < WARNING_THRESHOLD else 
                             'WARNING' if file_count < CRITICAL_THRESHOLD else 
                             'CRITICAL'
                }
...
def generate_report(results):
    ...
    for domain, data in sorted_results:
        ...
        if status == 'CRITICAL':
            ...
            print(f"   Path: {path}")
            print(f"   Action needed: Split into subdirectories immediately!")
            critical_count += 1
        elif status == 'WARNING':
            print(f"⚠️  WARNING: {domain} ({count} files) - Approaching limit!")
            print(f"   Path: {path}")
            print(f"   Consider splitting soon")
            warning_count += 1
        else:
