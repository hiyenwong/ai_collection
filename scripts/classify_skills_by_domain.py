...
def analyze_skills_distribution():
    ...
    for domain in sorted(
        domain_stats.keys(), key=lambda x: len(domain_stats[x]), reverse=True
    ):
...
def generate_migration_plan(domain_stats: dict):
    """生成迁移计划"""
    plan = {"source": "collection/skills/", "target_structure": {}, "migrations": []}

    for domain, skills in domain_stats.items():
        target_dir = f"collection/skills/{domain}/"
        plan["target_structure"][domain] = {"count": len(skills), "path": target_dir}
