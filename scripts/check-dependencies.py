...
def check_python_package(package: str) -> tuple[bool, Optional[str]]:
    ...
    try:
        result = subprocess.run(
            ["pip3", "show", package], capture_output=True, text=True, timeout=30
        )
        if result.returncode == 0:
            # Extract version
...
def check_skill(skill: Skill, system: SystemInfo) -> Skill:
    ...
    skill.dependencies = [check_dependency(dep, system) for dep in skill.dependencies]
    return skill
...
def get_install_command(dep: Dependency, system: SystemInfo) -> Optional[str]:
    ...
    if dep.category == "system":
        ...
        return system.install_commands.get("pip", "pip3 install {package}").format(
            package=dep.name
        )

    elif dep.category == "npm":
        return system.install_commands.get("npm", "npm install -g {package}").format(
            package=dep.name
        )
...
def install_dependency(
    dep: Dependency, system: SystemInfo, dry_run: bool = False
) -> bool:
    ...
    if not cmd:
        print_warning(
            f"No install command available for {dep.name} on {system.os_name}"
        )
        return False
    ...
    try:
        ...
        if (
            "externally-managed-environment" in result.stderr
            or "--break-system-packages" in result.stderr
        ):
            print_warning(
                "System Python is externally managed. Trying alternative methods..."
            )
            ...
            result2 = subprocess.run(
                alt_cmd, shell=True, capture_output=True, text=True, timeout=300
            )

            if result2.returncode == 0:
                print_success(
                    f"Successfully installed {dep.name} (with --break-system-packages)"
                )
                return True
            else:
                print_error(f"Failed to install {dep.name}: {result2.stderr}")
                print_info(
                    "Consider using a virtual environment or pipx for Python packages."
                )
                return False
...
def print_dependency_status(dep: Dependency):
    ...
    status_color = (
        Colors.GREEN
        if dep.installed
        else (Colors.YELLOW if dep.optional else Colors.RED)
    )

    version_info = f" ({dep.version_installed})" if dep.version_installed else ""
    req_info = (
        f" [required: {dep.version_required}]"
        if dep.version_required and not dep.installed
        else ""
    )

    print(
        f"    {status_color}{status_icon} {dep.name}{Colors.RESET}{version_info}{req_info}"
    )
...
Examples:
  ...
  python scripts/check-dependencies.py --report     # Generate JSON report
    ...
    parser.add_argument(
        "--install", action="store_true", help="Auto-install missing dependencies"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    parser.add_argument(
        "--skill", type=str, help="Check a specific skill's dependencies"
    )
    ...
    print(
        f"  {Colors.BOLD}Satisfied:{Colors.RESET} {Colors.GREEN}{satisfied_deps}{Colors.RESET}"
    )

    if missing_required:
        print(
            f"  {Colors.BOLD}Missing (Required):{Colors.RESET} {Colors.RED}{len(missing_required)}{Colors.RESET}"
        )
    ...
    if args.install or args.dry_run:
        if missing_required:
            ...
            if args.dry_run:
                print_info(
                    f"[dry-run] Would install {len(missing_required)} dependencies"
                )
            else:
                print_info(
                    f"Successfully installed {success_count}/{len(missing_required)} dependencies"
                )
        else:
