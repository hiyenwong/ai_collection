...
def find_entry_by_id(index_data, arxiv_id):
    """Find an entry in the index list by arxiv_id"""
    for entry in index_data:
        if isinstance(entry, dict) and entry.get('id') == arxiv_id:
            return entry
    return None
...
def main():
    ...
    # Load existing index.json (as list)
...
"""
    ...
    for paper in papers:
        ...
        category, _ = found
        # Update index.json
        entry = find_entry_by_id(index_data, arxiv_id)
        if entry:
            # Update the entry
            entry['file'] = f"collection/skills/{category}/{skill_dir_name}/SKILL.md"
            entry['category'] = category
            # Ensure skillCreated is set to the directory name (should already be)
            entry['skillCreated'] = skill_dir_name
            updated_count += 1
        else:
            # Add new entry
            new_entry = {
