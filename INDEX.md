...
- [[qldpc-full-extractor-construction]] - Full extractor construction for measuring arbitrary logical Pauli operators on QLDPC code blocks enabling PBC without compilation overhead (arXiv: 2606.03507)
  - Extractors 50-80pct size of base HGP codes with max qubit degree 10
  - Circuit-level noise: logical error 1e-6 at 0.1pct physical error rate
...
- [[geometric-decoherence-time-lindbladian]] - Geometric decoherence time methodology defining the moment logarithmic negativity/Renyi entropy relation breaks down in open quantum systems (arXiv: 2606.02743)
...
- [[binary-spiking-causal-models]] - 二值脉冲神经网络的因果分析方法：将BSNN表示为二值因果模型，使用SAT/SMT求解器计算溯因解释，提供可解释性保证优于SHAP (arXiv: 2604.27007)
  - 二值因果模型：神经元脉冲状态作为二值变量，网络连接作为因果机制
  - 溯因解释：SAT求解器找到最小输入集解释输出，SMT求解器支持数值约束
  - MNIST验证：逻辑基解释保证不包含无关特征，优于SHAP
  - **Activation**: binary spiking, causal explanation, BSNN, spiking interpretability, SAT-based explanation, SMT solver, abductive explanation, neural explainability, interpretable SNN

### EEG-FuseFormer: Transformer-Driven Feature Fusion for Seizure Prediction
- [[eeg-fuseformer-seizure-prediction]] - Transformer驱动的EEG特征融合框架用于癫痫发作预测：CNN-LSTM（原始信号）+ ResNet-18（STFT域）双分支特征提取，Transformer编码器融合，CHB-MIT数据集98.85%召回率 (arXiv: 2606.02166)
  - 双分支特征：原始EEG信号（空间-时序）+ STFT时频谱图（频域）
  - Transformer融合：中间特征融合学习跨域交互，优于后期拼接
  - 跨患者适应：目标患者少量数据微调提升3-5%性能
  - 临床优先召回：98.85%召回率避免漏检癫痫发作
  - **Activation**: EEG seizure prediction, feature fusion, transformer EEG, CNN-LSTM EEG, STFT EEG, epilepsy prediction, CHB-MIT dataset, cross-patient generalization, medical AI
