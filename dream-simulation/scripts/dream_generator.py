...
class DreamGenerator:
    ...
    # 超现实元素
    SURREAL_ELEMENTS = [
        "物体漂浮在空中", "重力消失", "时间倒流", "空间折叠",
        "物体改变颜色", "人可以穿过墙壁", "动物会说话",
        "建筑在生长", "镜子中的另一个世界"
    ]
    ...
    def consolidate_memories(self, fragments: List[MemoryFragment]) -> List[MemoryFragment]:
    ...
    def construct_narrative(self, fragments: List[MemoryFragment]) -> str:
        ...
        symbols = self.DREAM_SYMBOLS.get(dominant_emotion, self.DREAM_SYMBOLS["curiosity"])
        ...
        for i, frag in enumerate(fragments[:5]):  # 最多处理5个片段
            if random.random() < self.params.narrative_coherence:
                # 连贯叙事
                narrative_parts.append(self._coherent_scene(frag, symbols[i % len(symbols)]))
            else:
                # 跳跃式叙事 (梦境特征)
    ...
    def _generate_opening(self, emotion: str) -> str:
        ...
        openings = {
            ...
            "curiosity": ["你发现了一扇神秘的门", "你进入了一个未知的世界", "你拿着一张古老的地图"],
            "stress": ["机器在你身边嗡嗡作响", "你在无尽的走廊中奔跑", "时间在你眼前加速流逝"]
        }
    ...
    def _incoherent_scene(self, fragment: MemoryFragment) -> str:
        ...
        return f"场景变换，{surreal}，你感到{fragment.emotional_valence:.1f}的情感波动。"
