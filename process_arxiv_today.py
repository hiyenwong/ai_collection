...
def parse_fetch_output(output):
    ...
    while i < len(lines):
        ...
        if match:
            ...
            url = ''
            if i+1 < len(lines):
                t_line = lines[i+1].strip()
                if t_line.startswith('Title:'):
                    title = t_line[6:].strip()
            if i+2 < len(lines):
                a_line = lines[i+2].strip()
                if a_line.startswith('Authors:'):
                    authors = a_line[8:].strip()
            if i+3 < len(lines):
                u_line = lines[i+3].strip()
                if u_line.startswith('URL:'):
                    url = u_line[4:].strip()
            papers.append({
                'rank': rank,
                'id': paper_id,
            ...
            i += 5  # skip utility line and the next four lines (title, authors, url, empty line)
        else:
            i += 1
    return papers
...
def main():
    ...
    # Create a set of existing arxiv_ids in index_data for quick lookup
    existing_ids = {item.get('id') for item in index_data if isinstance(item, dict)}
...
"""
    ...
    # Update index.json: add new entries for papers not already present
    updated_count = 0
    for paper in papers:
        arxiv_id = paper['id']
        if arxiv_id in existing_ids:
            # Already in index.json, we can optionally update the entry, but we skip for now.
            continue
        skill_dir_name = get_skill_dir_name(arxiv_id, paper['title'])
        category = classify_skill(skill_dir_name)
        if category is None:
        ...
        updated_count += 1
        existing_ids.add(arxiv_id)  # avoid adding same paper twice in this loop

    if updated_count > 0:
        save_index_json(index_data)
        print(f"Added {updated_count} new entries to {INDEX_JSON}.")
    else:
        print("No new entries to add to index.json.")

    # Now update INDEX.md with a section for today
    # We'll use the papers list to get the correct category and skill_dir_name.
    # Let's build the mapping from arxiv_id to (category, skill_dir_name) by scanning again.
    paper_info = {}
    for paper in papers:
        arxiv_id = paper['id']
        skill_dir_name = get_skill_dir_name(arxiv_id, paper['title'])
